...
public class BemerkungUtils {
	...
	private static Bemerkung leereBemerkung(Sos sos, int epk_id) {
		...
		bem.setText("");
		try {
			R.DB.bemerkungDao.assignEmptyForeignCollection(bem,
					"unterschriften");
	...
	public static EpkBemEinzelController createLeereBemerkungController(
			EpkController epkController) {
		Bemerkung leereBem = leereBemerkung(R.State.sos, R.State.epk.getId());
		EpkBemEinzelController contr = new EpkBemEinzelController(leereBem,
				epkController);
		contr.setNeu();
		return contr;
	}

	public static Bemerkung insertBemerkung(Bemerkung quellBem,
			BemerkungSuchErgebnis ergebnis) {

		if (ergebnis.inEigenerEpkGefunden()) {
			unterschreibe(ergebnis.getBem(), R.State.lehrer);
			return null;
		} else if (ergebnis.nurInAndererEpkGefunden()) {
			Bemerkung neuBem = bemAusText(quellBem.getText(),
					quellBem.getSos(), quellBem.getEpk_id());
	...
	private static Bemerkung insertAlsZitat(Bemerkung zitBem) {
		...
		Notifications.create().title("Bemerkung übernommen")
		.text("Bemerkung wurde in aktuelle Epk übernommen.")
		.hideAfter(new Duration(3000)).position(Pos.BOTTOM_CENTER)
		.showInformation();
	...
	private static void unterschreibe(Bemerkung bem, Lehrer lehrer) {
		...
		if (bem.isUnterzeichner(lehrer)) {
			Notifications.create().title("Aktion nicht möglich")
			.text("Diese Bemerkung ist schon unterschrieben")
			.hideAfter(new Duration(3000)).position(Pos.BOTTOM_CENTER)
			.showInformation();
		} else {
