...
public class SelectLehrerController extends AbstractSelectViewController<SelectLehrerView, Lehrer> {
	...
	private ObservableList<Lehrer> leseLehrerListe() {
		...
		List<Lehrer> list = admin ? R.DB.lehrerDao.queryForEq("admin", true)
	...
	public void newSelection(Lehrer lehrer) {
		...
		String message = "Hallo " + lehrer.getAnrede() + " " + lehrer.getName() + "," + "\n"
				+ "Sie benutzen das Programm nun als " + lehrer.getKuerzel() + ".";

		String title = "Lehrerauswahl";

		Dialogs.create().title(title).message(message).masthead(null).showInformation();
